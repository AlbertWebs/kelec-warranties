        ...
        <table class="min-w-full text-left text-sm">
            <thead>
                <tr class="border-b border-slate-200 bg-slate-50/80 text-xs font-semibold uppercase tracking-wide text-slate-500">
                    ...
                    <th class="px-4 py-3">Mobile (OTP)</th>
                    <th class="px-4 py-3">Role</th>
                    <th class="px-4 py-3">Active</th>
                    <th class="px-4 py-3"><span class="sr-only">Update</span></th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
                @forelse ($users as $user)
                    <tr class="align-top transition hover:bg-brand-soft/80">
                        <td class="px-4 py-3.5 font-medium text-brand-ink" colspan="6">
                            <form method="POST" action="{{ route('admin.users.update', $user) }}" class="grid gap-2 md:grid-cols-6 md:items-center">
                                @csrf
                                @method('PUT')
                ...
                @empty
                    <tr><td colspan="6" class="px-4 py-16 text-center text-sm text-slate-500">No users found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
