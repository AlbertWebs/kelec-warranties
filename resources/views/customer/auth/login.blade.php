<x-guest-layout>
    <div class="mb-5">
        ...
        <h1 class="mt-1 text-xl font-bold text-brand-ink sm:text-2xl">Welcome back</h1>
        <p class="mt-1 text-sm text-gray-600">
            Sign in to view warranty history and manage claims.
        </p>
    </div>

    <x-auth-session-status class="mb-3" :status="session('status')" />

    <form method="POST" action="{{ route('customer.login.store') }}" class="space-y-3" x-data="{ showPassword: false }">
        @csrf

        <div class="auth-field">
            <label for="login" class="auth-label">Email or mobile number</label>
            <input
                id="login"
                class="auth-input"
                type="text"
            ...
            >
            <x-input-error :messages="$errors->get('login')" class="!mt-0" />
        </div>
    ...
    <div class="mt-5 border-t border-gray-100 pt-4 text-center text-sm text-gray-600">
        New here?
        <a href="{{ route('customer.register') }}" class="font-semibold text-brand hover:text-brand-dark underline underline-offset-2">Create a customer account</a>
    </div>
</x-guest-layout>
